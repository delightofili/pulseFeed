...
export function getMessages(userId1, userId2) {
  return db
    .prepare(
      `
    SELECT messages.*, sender.name as sender_name, sender.username as sender_username, sender.avatar as sender_avatar FROM messages JOIN users as sender ON messages.sender_id = sender.id WHERE (sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?) ORDER BY created_at ASC
    `,
    )
    .all(userId1, userId2, userId2, userId1);
}

export function sendMessagesDb({ senderId, receiverId, content }) {
  db.prepare(
    "INSERT INTO messages (sender_id, receiver_id, content, created_at) VALUES (?, ?, ?, ?)",
  ).run(senderId, receiverId, content, new Date().toISOString());
}
...
export function getConversations(userId) {
  ...
  `;
  return db.prepare(query).all(userId, userId, userId, userId);
}
